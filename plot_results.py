...
def main():
    ...
    ax.plot(episodes_agent, agent_scores, marker="o", label="Agente DP", **line_kwargs)
    ax.plot(episodes_random, random_scores, marker="o", label="Aleatorio", **line_kwargs)
    ax.axhline(stats.mean(agent_scores), color="#1f77b4", linestyle="--", alpha=0.6, label="Media agente")
    ax.axhline(stats.mean(random_scores), color="#d62728", linestyle="--", alpha=0.6, label="Media aleatorio")
